...
export default function Header({ language, onLanguageChange, onHomeClick }) {
    return (
        <header className="glass-card sticky top-0 z-50 border-b border-white/5">
            <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between">
                ...
                <button
                    type="button"
                    onClick={onHomeClick}
                    className="flex items-center gap-3 text-left rounded-xl px-1 py-1 hover:bg-white/5 transition-colors"
                    title={language === 'hi' ? 'होम पर जाएं' : 'Go to home'}
                    aria-label={language === 'hi' ? 'होम पर जाएं' : 'Go to home'}
                >
