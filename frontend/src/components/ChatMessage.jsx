 ...
 * fast line-by-line reveal animation for better perceived speed.
 */
...
/**
 * Hook that reveals text line by line (sentence chunks).
 * @param {string} fullText - The complete text to reveal.
 * @param {boolean} shouldAnimate - Whether to animate or show instantly.
 * @param {number} speed - Milliseconds between each line (default 45ms).
 * @returns {{ displayedText: string, isTyping: boolean }}
 */
function useLineReveal(fullText, shouldAnimate, speed = 45) {
    ...
    useEffect(() => {
        ...
        if (explicitLines.length > 1) {
            lines.current = explicitLines;
        } else {
            const sentenceChunks = normalized.match(/[^.!?\n]+[.!?]?/g) || [normalized];
            lines.current = sentenceChunks.map((chunk) => chunk.trim()).filter(Boolean);
        }

        if (!shouldAnimate) {
            setLineIndex(lines.current.length);
            setIsTyping(false);
            return;
        }
    ...
    useEffect(() => {
        if (!isTyping || lineIndex >= lines.current.length) {
            if (lineIndex >= lines.current.length && lines.current.length > 0) {
                setIsTyping(false);
            }
            return;
        }
        ...
        const timer = setTimeout(() => {
            setLineIndex((prev) => Math.min(prev + 1, lines.current.length));
        }, speed);

        return () => clearTimeout(timer);
    }, [lineIndex, isTyping, speed]);

    const displayedText = lines.current.slice(0, lineIndex).join('\n');
    return { displayedText, isTyping };
}

// ══════════════════════════════════════════════════════════════════════════
// LINE REVEAL TEXT COMPONENT
// ══════════════════════════════════════════════════════════════════════════

function LineRevealText({ text, animate, className, onFinished, speed = 45 }) {
    const { displayedText, isTyping } = useLineReveal(text, animate, speed);
...
export default function ChatMessage({ message, language, isLatest = false }) {
    ...
    // Track whether sections under summary are visible.
    const [revealedSections, setRevealedSections] = useState(
        isLatest ? 0 : 99  // if not latest, show everything instantly
    );

    // Show all supporting sections as soon as summary animation finishes.
    const revealNext = useCallback(() => {
        setRevealedSections(5);
    }, []);
    ...
    // While streaming, keep only summary visible. When streaming ends without animation,
    ...
    useEffect(() => {
        ...
        if (isLatest && !message.hasStreamed) {
            return;
        }
        if (isLatest && message.hasStreamed) {
            setRevealedSections(5);
        }
    }, [isStreaming, isLatest, message.hasStreamed]);
    ...
    const shouldAnimate = isLatest && !isStreaming && !message.hasStreamed;
    ...
    return (
        <div className="flex items-start gap-3 animate-slide-up">
            ...
            <div className={`flex-1 glass-card rounded-2xl rounded-tl-md overflow-hidden ${isHindi ? 'hindi-text' : ''}`}>
                {/* Summary — animated line by line for faster perceived response */}
                {data.summary && (
